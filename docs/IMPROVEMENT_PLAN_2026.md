...
| 2026-07-26 | 用户头像系统（端到端）：①DB `users` 表加 `avatar_type`/`avatar_ref` 列 + ALTER TABLE 回填（老用户默认 preset/01）；②`avatarRoutes.ts`（新）4 端点——GET 预设列表 / GET `/api/avatar/:userId` 读取上传头像 / POST 选预设 / POST 上传（multer 内存存储 → sharp 去 EXIF + 中心裁剪 256×256 WebP → 写盘 `data/avatars/{userId}.webp`，与 SQLite 同级持久化区）；③`Avatar` 类型提到 `citadels-common` 单一来源，server/client 共用；④avatar 在 `attachAuth` 存入 socket，贯穿 reconnect/addPlayer/'add player' 事件 → `Player.avatar` → `GameState.getStateFromPlayer` → `ClientGameState.players[].avatar`；⑤客户端 `avatarUrl` 工具按 type→ref 解析 URL，AuthPanel profile modal 头像选择区（预设网格 + 文件上传），登录后头部 / SeatPanel 座位面板 / PlayersList 大厅座位卡 / EndGameModal 终局积分表渲染头像。预设头像 14 个（`01.png`~`14.png`）位于 `client-react/public/avatars/`（进 git 随客户端打包），上传头像位于 `data/avatars/`（gitignore 运行时生成）。依赖新增 multer + sharp + @types/multer |
| 2026-07-26 | 修复 `statusBar.test.ts` 预存类型错误与 1 项失败测试：`makeSelfBoard` 返回类型未标注（→ `PlayerBoard`，参数 `string[]`→`DistrictId[]`，修 8 处 not-assignable）；`CharacterType.GRAVEYARD_RECOVER_DISTRICT` 误用（该枚举值属 `ClientTurnState`，→ 改 `CharacterType.NONE`）；`selected` 推断 `string[]`（→ 标注 `DistrictId[]`）；「INITIAL→welcome」测试 setup 与意图不符（默认 currentPlayer=PLAYER_1 在范围内走 choose_card 而非 welcome 分支，→ 覆写 `PlayerPosition.SPECTATOR` 越界走 welcome）。common 测试 126→127 全绿，typecheck 干净 |
| 2026-07-26 | i18n 措辞 + 观战视觉修复：①分数条/终局「敌方」→「对方」（zh）/「Enemies」→「Opponents」（en），影响 `ui.team.enemy` 键（`GameTopBar`/`EndGameModal`）；②观战模式座位框原先全蓝（`getRelation` 在 spectator 时无脑返回 `'self'`→`seat-panel--ally`），改为按 player 实际 team 区分（Team A→ally 蓝 / Team B→enemy 红，team 缺失时按座位序号奇偶回退）；③`SeatPanel` 加 `isSpectator` prop，观战时 ally/enemy 标签从「队友」/「对手」改为「队 A」/「队 B」（观战者无队友，语义更准）；④`boardLayout.test.ts` 更新观战 relation 断言 + 新增 parity 回退测试。common 127→128、server 78 测试全绿 |
