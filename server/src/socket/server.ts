...
const EMPTY_ROOM_CHECK_INTERVAL_MS = 60 * 1000;
const CHAT_MESSAGE_MAX_LENGTH = 200;

setInterval(() => {
  const now = Date.now();
  for (const room of gameStore.findAllRooms()) {
    if (room.gameState.emptySince && now - room.gameState.emptySince > EMPTY_ROOM_TIMEOUT_MS) {
      disposeTurnTimer(room.roomId);
      gameStore.removeRoom(room.roomId);
      debug(`evacuated empty room ${room.roomId} after timeout`);
    }
  }
}, EMPTY_ROOM_CHECK_INTERVAL_MS);
...
function leaveRoomForPlayer(room: Room, playerId: PlayerId) {
  ...
  room.update();
  if (!room.gameState.hasHumanPlayers()) {
    setImmediate(() => {
      disposeTurnTimer(room.roomId);
      gameStore.removeRoom(room.roomId);
      debug(`removed empty room ${room.roomId}`);
    });
  }
}

function safeLeaveRoom(room: Room, playerId: PlayerId, callback: (res: any) => void) {
  try {
    leaveRoomForPlayer(room, playerId);
    callback({ status: 'ok' });
  } catch (err) {
    console.error('[leave room] failed', err);
    callback({ status: 'error', message: 'leave room failed' });
  }
}
  ...
  io.on('connection', (socket: ExtendedSocket) => {
      ...
      try {
        ...
        if (socket.roomId && socket.playerId) {
          ...
          if (room) {
            ...
            setImmediate(() => {
              try {
                const roomAfter = gameStore.findRoom(roomId);
                if (!roomAfter) return;
                const hasPlayers = Array.from(roomAfter.gameState.players.values())
                ...
                console.error('[disconnect] setImmediate cleanup failed', err);
              }
            });
          }
        }
    ...
    socket.on('create room', (callback) => {
      try {
        ...
        if (socket.playerId) {
          previousRoom = gameStore.findRoomByPlayerId(socket.playerId);
        ...
        if (previousRoom && socket.playerId) {
          leaveRoomForPlayer(previousRoom, socket.playerId);
        }
        ...
        console.error('[create room] handler failed', err);
        callback({ status: 'error', message: 'internal server error' });
      }
    });
    ...
    socket.on('get room info', (roomId, callback) => {
      try {
        ...
        console.error('[get room info] handler failed', err);
        callback({ status: 'error', message: 'internal server error' });
      }
    });
    ...
    socket.on('join room', (
      ...
      try {
        ...
        // enforce one room per player: if already in another room, leave it first
        if (socket.userId && socket.playerId) {
          const existingRoom = gameStore.findRoomByPlayerId(socket.playerId);
          if (existingRoom && existingRoom.roomId !== roomId) {
            leaveRoomForPlayer(existingRoom, socket.playerId);
          }
        }
        ...
        console.error('[join room] handler failed', err);
        const cb = typeof asSpectator === 'function' ? asSpectator : callback;
        cb({ status: 'error', message: 'internal server error' });
      }
    });
    ...
    socket.on('add ai player', (callback) => {
      try {
        ...
        console.error('[add ai player] handler failed', err);
        callback({ status: 'error', message: 'internal server error' });
      }
    });
    ...
    socket.on('remove ai player', (playerId: PlayerId, callback) => {
      try {
        ...
        console.error('[remove ai player] handler failed', err);
        callback({ status: 'error', message: 'internal server error' });
      }
    });
    ...
    socket.on('start game', (serializedGameSetupData, callback) => {
      try {
        ...
        console.error('[start game] handler failed', err);
        callback({ status: 'error', message: 'internal server error' });
      }
    });
    ...
    socket.on('set lobby role', (roleRaw: string, callback) => {
      try {
        ...
        console.error('[set lobby role] handler failed', err);
        callback({ status: 'error', message: 'internal server error' });
      }
    });
    ...
    socket.on('set autoplay', (enabled: boolean, callback) => {
      try {
        ...
        console.error('[set autoplay] handler failed', err);
        callback({ status: 'error', message: 'internal server error' });
      }
    });
    ...
    socket.on('chat message', (msg: string, callback) => {
      try {
        ...
        const text = String(msg).trim().slice(0, CHAT_MESSAGE_MAX_LENGTH);
        if (!text) {
    ...
    socket.on('make move', (move: Move, callback) => {
      try {
        ...
        console.error('[make move] handler failed', err);
        callback({ status: 'error', message: 'internal server error' });
      }
    });
  });
